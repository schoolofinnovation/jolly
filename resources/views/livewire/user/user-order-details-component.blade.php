<main>
<div class=" container px-4 pt-lg-3 pb-3 mb-5">
          ...
          <div class="tab-pane fade active show" id="specs" role="tabpanel">
            @foreach($order->orderItems as $fun)
              <div class="d-md-flex justify-content-between align-items-start pb-4 mb-4 border-bottom">
                <div class="d-flex align-items-center me-md-3"><img src="{{url('Storage/brands/'.$fun->franchise->image)}}" width="90" alt="{{Str::limit($fun->franchise->brand_name, 24)}}"> 
                  <div class="ps-3">
                    <h6 class="fs-base mb-2">{{$fun->franchise->brand_name}}</h6>
                    <div class="h4 fw-normal text-accent">{{$fun->franchise->max_investment}} -  <small>{{$fun->franchise->min_investment}}</small> Lacs. </div>                      
                  </div>
                </div>
                <div class="d-flex align-items-center pt-3">
                  ...
                  @if($order->status == 'delivered' && $fun->rstatus == false)
                  <a class="btn btn-primary btn-sm btn-shadow me-2" type="button" href="{{route('user.review',['order_item_id' => $fun->id])}}">
                    <span class="d-none d-sm-inline">Write Review</span></a>
                  @endif
                  @if($order->status == 'ordered')
                  <button class="btn btn-primary btn-shadow me-2 btn-sm" type="button" href="#" wire:click.prevent="cancelOrder">
                    <span class="d-none d-sm-inline">Cancel Order</span></button>
                  @endif
                  <button class="btn btn-outline-primary btn-shadow me-2" type="button" href="#" >
                    <span class="d-none d-sm-inline">{{ucwords(trans($fun->order->status))}}</span></button>

                  <div class="me-2">
                    <button class="btn btn-secondary btn-icon" type="button" href="#" wire:click.prevent="index({{$fun->id}})" data-bs-toggle="tooltip" title="" data-bs-original-title="Download" aria-label="Download"><i class="bi bi-arrow-up fs-lg"></i></button>
                  </div>
            ...
            @endforeach
                ...
                <div class="row pt-2">
                  <div class="col-lg-5 col-sm-6">
                    <h3 class="h6">Product Detials</h3>
                    <ul class="list-unstyled fs-sm pb-2">
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Order ID: </span><span>{{$order->id}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Order Date:</span><span>{{$order->created_at}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Status :</span><span>{{$order->status}}</span></li>
                      @if($order->status == 'delivered')
                          <li class="d-flex justify-content-between pb-2 border-bottom">
                            <span class="text-muted">Delivery Date:</span><span>{{$order->delivered_date}}</span></li>
                      @elseif($order->status == 'canceled')
                          <li class="d-flex justify-content-between pb-2 border-bottom">
                            <span class="text-muted">Canceled Date:</span><span>{{$order->canceled_date}}</span></li>
                      @endif
                    </ul>
                  ...
                  <div class="col-lg-5 col-sm-6 offset-lg-1">
                    ...
                    <ul class="list-unstyled fs-sm pb-2">
                      <li class="d-flex justify-content-between pb-2 border-bottom">
                        <span class="text-muted">  @foreach($order->orderItems as $fun) {{ucwords(trans($fun->franchise->brand_name))}} :</span>
                      <span>{{$fun->franchise->max_investment}} X  Per Unit @endforeach</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Discount:</span><span> Applied Code : {{$order->code}} | {{$order->discount}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Subtotal:</span><span>Rs.{{$order->subtotal}} + Taxes : 18% ( {{$order->tax}} )</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Total:</span><span>Rs.{{$order->total}} </span></li>
                    </ul>
                    <h3 class="h6">Billing Detials</h3>
                    <ul class="list-unstyled fs-sm pb-2">
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Name:</span><span>{{ucwords(trans($order->firstname))}} {{ucwords(trans($order->lastname))}}  </span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Location:</span><span>{{ucwords(trans($order->city))}} | {{ucwords(trans($order->province))}} | {{ucwords(trans($order->country))}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Contact:</span><span>{{$order->mobile}} | {{$order->email}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Address:</span><span>{{$order->line1}}, {{$order->zipcode}} </span></li>
                    </ul>
                    @if($order -> is_shipping_different)
                    <h3 class="h6">Shipping Detials</h3>
                    <ul class="list-unstyled fs-sm pb-2">
                      
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Location:</span><span>{{ucwords(trans($order->shipping->city))}} | {{ucwords(trans($order->shipping->province))}} | {{ucwords(trans($order->shipping->country))}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Contact:</span><span>{{$order->shipping->mobile}} | {{$order->shipping->email}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Address:</span><span>{{$order->shipping->line1}}, {{$order->shipping->zipcode}} </span></li>
                    </ul>
                    @endif
                     
                    <h3 class="h6">Transaction</h3>
                    <ul class="list-unstyled fs-sm pb-2">
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Mode of Payment:</span><span>{{ucwords(trans($order->transaction->mode))}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Transaction Status:</span><span>{{ucwords(trans($order->transaction->status))}}</span></li>
                      <li class="d-flex justify-content-between pb-2 border-bottom"><span class="text-muted">Transaction:</span><span>{{Carbon\Carbon::parse ($order->created_at)->format('D, d F Y  h:i:s')}}</span></li>
                    </ul>
                  </div>
                </div>
              </div>
