...
@section('page_path',' Add Opportunities')
@section('page_list',' addbrand')
@section('page_name',' Add Opportunities')

@section('page_title',' Make a Business Partner')

@section('page_description','Job')
@section('page_keywords', 'Council, Innovation, sell your business, market, expand your Opportunities, buy a brand licenese,  business_design, business_strategy, business_design_sprint, innovation_accelerator, product_service, go_to_market, entrepreneur_residence, strategy_sprint, creative')
          ...
          <div class="row mt-5" >
                    @if($currentStep == 1)
                          <div class="col-sm-3 mb-3">
                            <label class="form-label" >Brand {{$co_name}}  {{$tryin->count()}}
                              <a class="" href="{{route('seller.brand')}}">Create New Brand</a>
                            </label>
                    ...
                    @if($currentStep == 3)
                          ...
                          <div class="col-sm-4 mb-3">
                            ...
                            <div class=" col-sm-3  mb-3 input-group">
                              <select class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon" wire:model="attr">
                              <option selected >Choose...</option> 
                              @foreach ($pattribute as $cat)<option value="{{$cat->id}}" >{{$cat->name}}</option> @endforeach
                              </select>
                              <button class="btn btn-outline-primary" type="button" wire:click.prevent="addAttri">Add</button>
                            </div>
                              <span class="text-danger">@error( 'State' ){{ $message}}@enderror</span>
                           </div>
                            ...
                            <div class="col-sm-4 mb-1 pb-1 ">
                                <label class="form-label" >{{$pattribute->where('id',$attribute_arr[$key])->first()->name}}</label>
                                <div class="input-group mb-3">
                                <input class="form-control" type="text" aria-describedby="button-addon2"  placeholder="{{$pattribute->where('id',$attribute_arr[$key])->first()->name}}" wire:model="attribute_values.{{$value}}">                           
                                <button class="btn btn-outline-primary" type="button" wire:click.prevent="delAttri({{$key}})">Delete</button>
                             </div>
                            </div>
                            @endforeach
